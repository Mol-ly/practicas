/*Categoría por edad: Solicite la edad del usuario. 
Clasifíquelo en: niño (0-12), adolescente (13-17), adulto (18-64) o adulto mayor (65+).*/

import readline from 'node:readline';
const rl = readline.createInterface({
  input: process.stdin,
  output: process.stdout
});

/*pregunta al usuario para obtener la edad*/
rl.question('Ingrese su edad: ', (input) => {
  const edad = parseInt(input);
  
  /*clasifica la edad del usuario y muestra el resultado*/
    if (edad >= 0 && edad <= 12) {
        console.log('Usted es un niño.');
    } else if (edad >= 13 && edad <= 17) {
        console.log('Usted es un adolescente.');
    } else if (edad >= 18 && edad <= 64) {
        console.log('Usted es un adulto.');
    } else if (edad >= 65) {
        console.log('Usted es un adulto mayor.');
    } else {
        console.log('Edad inválida. Por favor, ingrese un número válido.');
    }
    
    rl.close();
});

